...
def load_config() -> Tuple[List[str], str]:
    ...
    return cookies_list, exchange_plan
...
def checkin_and_process(cookie: str, exchange_plan: str) -> Tuple[str, str, str, str, str, str]:
    ...
    try:
        ...
        if "Checkin! Got" in response_message:
            status_msg = f"成功"#签到成功: 获得 {points_gained} 积分"
        elif "Checkin Repeats!" in response_message:
            status_msg = f"重复"#重复签到, 明天再来"
            points_gained = "0"
        else:
            status_msg = f"失败"#签到失败: {response_message}"
            points_gained = "0"
    ...
    if status_response:
        try:
            ...
            email = status_data.get('data', {}).get('email', "未知邮箱")
    ...
    if current_points_numeric >= required_points:
        ...
        if exchange_response:
            try:
                ...
                if code == 0:
                    ...
                    exchange_msg = f"兑换失败 {code} {detailed_msg}"#{exchange_plan}, 错误代码: {code}, 详情: {detailed_msg}"
            except json.JSONDecodeError:
        ...
        sum = required_points - current_points_numeric
        exchange_msg = f"积分不足 差 {sum} 积分" #，未兑换: {exchange_plan}"
...
def format_push_content(results: List[Dict[str, str]]) -> Tuple[str, str]:
    ...
    for i, res in enumerate(results, 1):
        line_parts = [
            f"🔢 账号{i}: {res['email']}\n",
            f"📅 签到结果: {res['status']}，获得 {res['points']} 积分\n",
            f"💰 当前积分: {res['points_total']}\n",
            f"⏳ 剩余天数: {res['days']}\n",
            f"🎁 积分兑换: {res['exchange']}"
        ]
...
def main():
    try:
        cookies_list, exchange_plan = load_config()
